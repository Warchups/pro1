package com.pmm.simarro.pmdm_banco_llopischristian.API;

import android.content.Context;


import com.pmm.simarro.pmdm_banco_llopischristian.bd.MiBD;
import com.pmm.simarro.pmdm_banco_llopischristian.dao.CuentaDAO;
...
public class MiBancoOperacional {
    ...
    */
    public int transferencia(Movimiento movimientoTransferencia){
        ...
        if (!miBD.existeCuenta(movimientoTransferencia.getCuentaDestino().getBanco(), movimientoTransferencia.getCuentaDestino().getSucursal(),
                movimientoTransferencia.getCuentaDestino().getDc(), movimientoTransferencia.getCuentaDestino().getNumeroCuenta())) {
            return 1;
        }

        if (movimientoTransferencia.getImporte() > movimientoTransferencia.getCuentaOrigen().getSaldoActual()) {
            return 2;
        }


        Cuenta cuentaDestino = (Cuenta) miBD.getCuentaDAO().search(movimientoTransferencia.getCuentaDestino());

        float importe = movimientoTransferencia.getImporte();
        float saldoActualOrigen = movimientoTransferencia.getCuentaOrigen().getSaldoActual();
        float saldoActualDestino = movimientoTransferencia.getCuentaDestino().getSaldoActual();

        movimientoTransferencia.setCuentaDestino(cuentaDestino);
        movimientoTransferencia.setImporte(-movimientoTransferencia.getImporte());

        miBD.insercionMovimiento(movimientoTransferencia);


        movimientoTransferencia.getCuentaOrigen().setSaldoActual(saldoActualOrigen - importe);
        movimientoTransferencia.getCuentaDestino().setSaldoActual(saldoActualDestino + importe);
        ...
        Movimiento movimiento = new Movimiento(25, 1, new Date(), "Transferencia Usuario", importe, movimientoTransferencia.getCuentaDestino(), movimientoTransferencia.getCuentaOrigen());
        miBD.insercionMovimiento(movimiento);
